        ...
        <div className="grid grid-cols-3 gap-4">
          {job.items.map((item, index) => (
            <div key={item.id} className="aspect-square relative group">
              {/* Phase 4: Enhanced loading states and error handling */}
              {!item.url ? (
                <div className="w-full h-full bg-muted rounded-lg flex items-center justify-center">
                  <div className="text-center text-muted-foreground">
                    <div className="w-6 h-6 animate-spin rounded-full border-b-2 border-primary mx-auto mb-2" />
                ...
                <img
                  ...
                  onError={(e) => {
                    console.error(`❌ JobGrid: Image failed to load: ${item.url}`);
                    // Phase 2: Enhanced error fallback
                    e.currentTarget.style.display = 'none';
                    e.currentTarget.parentElement?.classList.add('flex', 'items-center', 'justify-center', 'bg-destructive/10', 'border-2', 'border-dashed', 'border-destructive/20');
                    e.currentTarget.parentElement?.insertAdjacentHTML('beforeend', `
                      <div class="text-center text-destructive text-xs">
                        <div class="mb-1">Failed to load</div>
                        <div class="text-[10px] opacity-70">Check signed URL</div>
                      </div>
                    `);
                  }}
                  onLoad={() => {
                    console.log('✅ JobGrid: Image loaded successfully for item:', item.id);
                  }}
                  onClick={() => {
