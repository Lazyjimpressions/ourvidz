...
interface EnhancedReferenceSettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  references: Array<{
    id: string;
...
export const EnhancedReferenceSettingsModal = ({
  isOpen,
  onClose,
  references,
  onReferencesChange,
  ...
  onOptimizeChange
}: EnhancedReferenceSettingsModalProps) => {
  const { isDragging, draggedAsset } = useDrag();
  const [draggedOver, setDraggedOver] = useState<string | null>(null);

  // Auto-close modal if drag ends and modal was auto-opened
  useEffect(() => {
    if (!isDragging && !draggedAsset && isOpen) {
      // Add small delay to allow for drop completion
      const timeout = setTimeout(() => {
        // Only auto-close if no references are active (indicating it was auto-opened)
        const hasActiveRefs = references.some(ref => ref.enabled && ref.url);
        if (!hasActiveRefs) {
          onClose();
        }
      }, 500);
  ...
  }, [isDragging, draggedAsset, isOpen, references, onClose]);
              ...
              <div key={reference.id} className="space-y-2">
                ...
                <div
                  className={cn(
                    ...
                    (isDragging || draggedAsset) && "border-blue-300 bg-blue-300/5"
                  )}
                  onDragOver={(e) => {
                    e.preventDefault();
                ...
                >
                  {reference.enabled && reference.url ? (
                    ...
                    <div className={cn(
                      "flex flex-col items-center justify-center h-24 text-gray-400 transition-colors",
                      draggedOver === reference.id && "text-blue-400",
                      (isDragging || draggedAsset) && "text-blue-300"
                    )}>
                      <Upload className="w-6 h-6 mb-1" />
                      <p className="text-xs text-center">
                        {draggedOver === reference.id ? 'Drop here' : 'Drop image or click'}
                      </p>
                    </div>
                  )}
                </div>
