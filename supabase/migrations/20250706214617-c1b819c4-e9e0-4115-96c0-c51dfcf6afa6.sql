...
SET 
  status = 'failed',
  metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object('failed_at', now()::text)
FROM jobs 
...
SELECT 
  ...
  'system_recovery',
  jsonb_build_object(
    'operation', 'job_processing_recovery',
    'timestamp', now(),
    'description', 'Fixed stuck assets and purged legacy jobs'
  );