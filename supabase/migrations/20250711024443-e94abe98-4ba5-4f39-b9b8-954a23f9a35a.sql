...
CREATE VIEW public.content_moderation_analytics 
WITH (security_invoker = true) AS
SELECT 
...
ALTER VIEW public.user_analytics SET (security_barrier = true);
ALTER VIEW public.content_moderation_analytics SET (security_barrier = true);

-- Create RLS policies for user_analytics view (admin-only access)
CREATE POLICY "Admin can view user analytics" ON public.user_analytics
    FOR SELECT USING (has_role((SELECT auth.uid()), 'admin'::app_role));

-- Create RLS policies for content_moderation_analytics view (admin-only access) 
CREATE POLICY "Admin can view content moderation analytics" ON public.content_moderation_analytics
    FOR SELECT USING (has_role((SELECT auth.uid()), 'admin'::app_role));

-- Grant select permissions to authenticated users (policies will control actual access)
...
'User analytics view with SECURITY INVOKER - admin access only via RLS policies';
COMMENT ON VIEW public.content_moderation_analytics IS 
'Content moderation analytics view with SECURITY INVOKER - admin access only via RLS policies';