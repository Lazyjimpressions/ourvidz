...
async function generateSceneNarrativeWithOpenRouter(
  ...
  // ✅ FIX: Clean up narrative - remove common issues that cause unwanted elements
  // Remove asterisks (used for emphasis but may confuse the model)
  narrative = narrative.replace(/\*+/g, '');
  
  // Remove "A scene showing..." prefix (redundant since we already have "Generate a scene showing...")
  ...
  if (useI2IIteration) {
    // The reference image already maintains these, so we don't need to repeat them
    narrative = narrative
      ...
      .replace(/\bstill\s+(clad|dressed|wearing)\b/gi, '$1') // "still wearing" -> "wearing"
      .replace(/\s+/g, ' ') // Clean up multiple spaces
      .trim();
    
    console.log('🧹 Cleaned I2I narrative (removed asterisks, redundant maintain phrases, and scene prefix)');
  } else {
