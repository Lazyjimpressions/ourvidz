...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: currentJob, error: fetchError } = await supabase.from('jobs').select('metadata, job_type, image_id, video_id, format, quality, model_type').eq('id', jobId).single();
    if (fetchError) {
